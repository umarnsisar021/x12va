                        ...
                        />&emsp; {q.question}</p>
                            <div className="pb-3" style={{paddingLeft:'40px'}}>
                                {Object.values(q.options).map((option ,i)=>{
                                    return  <div className="form-check pb-2"> <input type="radio" id={`radio-${q.id}-${i}`} className="form-check-input"  {...register("answers["+`${q.id}`+"][option_id]",{required:true})}/> <label for={`radio-${q.id}-${i}`} className="form-check-label"> {option.option}</label></div>
                                })
                                }
                            </div>
                        </div>
