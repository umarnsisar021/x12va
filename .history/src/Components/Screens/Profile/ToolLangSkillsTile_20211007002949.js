...
export function SkillsTile(props) {
    ...
    const toggle2 = () => setTooltipOpen2(!tooltipOpen);
    ...
    return (
        <span className="toolLangSkills">
            ...
            { 
                props.status == 0 ?
                <span id={`TooltipTest-${props.id}`} className="icon-give-test" onClick={onGiveTest} style={{paddingLeft:'5px',paddingRight:'5px',float:'right' , borderLeft: "1px solid #D3D3D3"}}><Edit3 size={14}/></span>
                : null
            }
            {
                props.status == 0 ? <>
                    <Tooltip placement="top" isOpen={tooltipOpen} target={`TooltipNotVerified-${props.id}`} toggle={toggle}>
                        Skill Not Verified
                    </Tooltip>
                    <Tooltip placement="top" isOpen={tooltipOpen2} target={`TooltipTest-${props.id}`} toggle={toggle2}>
                        Start Test
                    </Tooltip>
                </> : '' 
           }
