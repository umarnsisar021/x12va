    ...
    useEffect(()=>{
        ...
        setTimeout(()=>{
            ...
            if(query.get('id')){
                ...
                useJwt.post('tasks/get_task_by_id',{task_id:query.get('id')}).then((res)=>{
                        ...
                        <div style={{ width: '100%', background: '#F7FFFD', padding: '60px 45px', border: '1px solid #7DC3B4', borderRadius: '5px'}}>
                            ...
                            <Row className="mt-3">
                                <div style={{display: 'flex',columnGap: '10px'}}>
                                    <button className="btn-theme-white btn-theme-sm btn-theme-border-default "><Link to={{pathname: '/experts/order/view/sendproposal', data: data}}>Send Proposal</Link></button>
                                </div>
                            </Row>
                        </div>
                    </div>
