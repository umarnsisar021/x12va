...
function RattingAndRemarks(props) {
    ...
    /// For Expert
        ...
        if(props.data.task_detail.status == 4){
            let rated = Object.values(props.data.ratings).map((row)=> { return props.data.expert.member_id === row.from_member_id ? true:false})
            return <div>
...

                        {
                            rated === true? null : 
                                <div className=" text-center">
                                        ...
                                        <p>Please leave your rating for <strong>{props.data.client.name}</strong>`s work.</p>
                                        <Rating onClick={handleRating}  ratingValue={rating} stars={5} />
                                        <textarea rows="2" placeholder="Remarks" onChange={(text)=>setRemarks(text.target.value)}>
                                            {remarks}
                                        </textarea>
