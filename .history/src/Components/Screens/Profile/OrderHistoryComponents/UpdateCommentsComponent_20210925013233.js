    ...
    return  <div>
        {
            comments.map((comment,c_index)=>{
                if(comment.member_id == props.userData.id){
                    let name = props.account_mode == "client" ? props.client.name : props.expert.name;
                    ...
                    let name2 = props.account_mode == "client" ? props.client.name : props.expert.name;
                    return <div className="p-2" key={c_index} style={{borderBottom:"1px solid lightgray"}}>
                                <p className="m-0 f-12" style={{fontWeight:500,color:"black"}}>{name2}</p>
                                <p className="m-0 f-12">{comment.comment}</p>
                            </div>
