...
export function SkillsTile(props) {
    return (
        <span className="toolLangSkills">
            {props.title}
            <span style={{paddingRight:'10px',}}>{props.status == 0 ? <AlertCircle size={14} color="#ff6d6d"/> : <Clock/>}</span>
            <span style={{float:'right' , borderLeft: "1px solid gray"}}><Clock size={14}/></span>
        </span>
    )
}
