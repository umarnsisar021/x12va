      ...
      const HandleStartTest =()=>{
         ...
         useJwt.post('test/start_test',{token:token, test_id: data.record.id}).then((res)=>{
            history.push({
                ...
                data: { questions: data.questions , test_attempt_id : res.data.test_attempt_id ,test_id : data.record.id, duration:data.record.duration }
            })
         })
      }
                    ...
                    <div className="mt-5" id="test-details-content" style={{display:'flex',columnGap: '50px'}}>
                        <div className="test-details-item">
                            ...
                            <span style={{ paddingTop: '3px'}}>
                                ...
                                <p className="my-0">{`${CalculateTime(data.record.duration)}`}</p>
...
const CalculateTime =  (hms) => {
    ...
    if(time.hours() > 0){
        ret += `${time.hours()} hours `
    }
    if(time.minutes() > 0){
        ret += `${time.minutes()} mins`
    }
