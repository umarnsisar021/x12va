...
async function getEducation(token) {
    let data = [];
     await useJwt.post('experts/get_my_educations',{token:token}).then((res)=>{
...
function ExpertEducationComponent  (props) {
    ...
    useEffect(()=>{
        setTimeout(async ()=>{
           let tools =  await getEducation(props.sessionToken);
           let temp = [];
    ...
    const  onSubmit = async (data)=>{
        ...
        data.expert_id = props.userData.id
        console.log(data)
        useJwt.post('experts/education_add',{...data,token:props.sessionToken}).then( async (res)=>{
            setLoaded(false);
            if(res){
    ...
    const deleteTool =  (id) => {
        useJwt.post('experts/education_delete',{id:id,token:props.sessionToken}).then( async (res)=>{
                toast.success(res.data.message,{})
                setLoaded(false);
                let tools =  await getEducation(props.sessionToken);
                setData(tools);
                        ...
                        <Modal.Body className="modal-body modal-content">
                            <div className="modal__wrapper">
                                ...
                                <form onSubmit={handleSubmit(onSubmit)}>
                                    ...
                                    <input autoComplete="off" type="text"  placeholder="Enter University/Institute Name" name="institute_name" defaultValue="" {...register("institute_name", { required: true })}/>
                                    {errors.institute_name && <error className="input-error"><AlertCircle color="#e87c7c" size={12} /> This field is required</error>}
                      
                                    <input autoComplete="off" type="text" placeholder="Degree Name" name="degree" defaultValue="" {...register("degree", { required: true })}/>
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => <Flatpickr placeholder="From" autoComplete="off" {...field} />}
                                    />
                                    ...
                                    <Controller
                                        ...
                                        render={({ field }) => <Flatpickr autoComplete="off"  placeholder="To" {...field} />}
                                    />
                                    {errors.date_to && <error className="input-error"><AlertCircle color="#e87c7c" size={12} /> This field is required</error>}
