...
function TransactionHistoryComponent(props) {
    ...
    React.useEffect(()=>{
        useJwt.post('transaction/get_transaction_history', {page: currentPage, perPage: rowsPerPage, token: props.sessionToken,status:1 }).then((res)=>{
            ...
            setTotalRows(res.data.records.total)
    ...
    const CustomPagination = (d) => {
        ...
        const count = Number(Math.ceil(totalRows / rowsPerPage))
        const row_start = (currentPage * rowsPerPage) - (rowsPerPage) + 1;
        const row_end = ((currentPage * rowsPerPage) - rowsPerPage) + data.length;
        return (
            <div >
                <span style={{ marginTop: '14px', fontSize: '14px', display: 'inline-block', color: '#A3A3A3' }}>Showing {row_start} - {row_end} of {totalRows}</span>
                <ReactPaginate
                    pageCount={3}
                    previousLabel={"<"}
                    nextLabel={">"}
    ...
    const columns = [
        {
            ...
            selector: 'Name',
            sortable: true,
            cell: row => <Moment format="MMM DD, YYYY h:mm a">{row.created_at}</Moment> 
        },
        {
        name: 'DESCRIPTION',
        minWidth: '160px',
