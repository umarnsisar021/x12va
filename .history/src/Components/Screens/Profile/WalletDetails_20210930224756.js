    ...
    const handlewithdraw = async () =>{
        ...
        useJwt.post("transaction/withdrawal",{token:props.sessionToken}).then((res)=>{
            ...
            if(error.response.data.status == 402){
                if(error.response.data.verify_link){
                    console.log(error.response.data.verify_link.url)
                    window.location = error.response.data.verify_link.url
                }
            ...
            if(error.response.data.status == 403){
                Swal.fire({
                    ...
                    html:'<span style="font-size: 16px">Your Balance seems too low to withdraw.</span>',
                })
                props.hideFadeLoader("")
            }
        })
        ...
        <React.Fragment>
            <div className="wrapper__main">
                ...
                <div className="refc__inner px-0 py-0" style={{boxShadow:'none',backgroundColor:'transparent'}}>
                    ...
                    <TransactionHistoryComponent callback={Run}  data={data.records}/>
