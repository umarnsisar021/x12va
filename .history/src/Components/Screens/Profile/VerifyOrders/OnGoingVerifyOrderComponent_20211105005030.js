...
function OnGoingVerifyOrderComponent(props) {
    ...
    React.useEffect(()=>{
        useJwt.post('experts/get_verify_ongoing', {page: currentPage, perPage: rowsPerPage, token: props.sessionToken    }).then((res)=>{
                    ...
                    <div className="wrapper__innerBox" style={{ padding: '0px', background: '#f6f7fa' }}>
                        ...
                        <div>
                            <DataTable
                                ...
                                data={dataToRender()}
                                onRowClicked={(row)=>{
                                    history.push('/experts/order/verify/history/'+row.id)
                                }}
                            />
                        </div>
...
export default connect(mapStateToProps,mapDispatchToProps)(OnGoingVerifyOrderComponent)
