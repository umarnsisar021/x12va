...
function ExpertEducationComponent  (props) {
    ...
    const  onSubmit = async (data)=>{
        ...
        date_from = `${date_from.getDate()}-${date_from.getMonth() + 1}-${date_from.getFullYear()}`
        date_to = `${date_to.getDate()}-${date_to.getMonth() + 1}-${date_to.getFullYear()}`
        data.date_from = date_from
        data.date_to = date_to
        data.expert_id = props.userData.id
        alert(props.userData.id)
