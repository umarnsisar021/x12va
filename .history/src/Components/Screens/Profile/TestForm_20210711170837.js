        ...
        <React.Fragment>
            <div className="wrapper__main">
                ...
                <div className="refc__inner" style={{padding:'40px'}}>
                    <div style={{position: 'fixed',right: 0,background: 'white',padding: '20px',boxShadow: '0px 2px 10px #eae9e9'}}>
                        {toSeconds('00:30:00')}
                        <Countdown date={Date.now() + 1800} renderer={({ hours, minutes, seconds, completed }) =>{
                            if (completed) {
                                // Render a completed state
                                return false;
                              } else {
                                // Render a countdown 
                                return <span>{minutes > 0 ? `${minutes} min`:''}:{seconds}</span>;
                              }
                        }} />
                    </div>
                        ...
                        />&emsp; {q.question}</p>
                            <div className="pb-3" style={{paddingLeft:'40px'}}>
                                {Object.values(q.options).map((option ,i)=>{
                                    return  <div className="form-check pb-2"> <input type="radio" className="form-check-input"/> <label className="form-check-label"> {option.option}</label></div>
                                })
                                }
                            </div>
                        </div>
...
const toSeconds = function (hms) {
    ...
    var a = hms.split(':'); // split it at the colons
 are worth 60 seconds. Hours are worth 60 minutes.
