...
function Proposals(props) {
    ...
    const assignTast = async (id) =>{
        await useJwt.post('clients/assign_task',{token:props.sessionToken,proposal_id:id})
        .then((res)=>{
            if(res){
                        ...
                        <div style={{ width: '100%', background: '#F7FFFD', padding: '60px 45px', border: '1px solid #7DC3B4', borderRadius: '5px'}}>
                            ...
                            <Row className="mt-3">
                                <div style={{display: 'flex',columnGap: '10px'}}>
                                    <button onClick={()=>assignTast()} className="btn-theme-white btn-theme-sm btn-theme-border-default ">Assign</button>
